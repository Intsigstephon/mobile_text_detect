...
class Block(nn.Module):
    ...
    def __init__(self, kernel_size, in_size, expand_size, out_size, nolinear, semodule, stride):
        ...
        if stride == 1 and in_size != out_size:    #it means: the feature map is not changed
            self.shortcut = nn.Sequential(
    ...
    def forward(self, x):
        ...
        out = out + self.shortcut(x) if self.stride==1 else out   #Actually,  out + shortcut
...
class MobileNetV3_Large(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.linear4(out)  #but no sigmoid
        return out
...
class MobileNetV3_Small(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        self.bneck = nn.Sequential(
            Block(3, 16, 16, 16, nn.ReLU(inplace=True), SeModule(16), 2),   #16 downsample
...
def test():
    ...
    #use tensorboardX  to get network architecutre
    dummy_input = Variable(torch.rand(1, 3, 224, 224))
    print("11111")
    w = SummaryWriter(comment='mobilenet_v3', log_dir="log")
    print("22222")
